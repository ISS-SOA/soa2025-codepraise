...
# NOTE: Modern browsers and WebDriver updates have changed page load timing behavior.
# Watir/Selenium may return control before all dynamic content (like flash messages)
# has finished rendering. Therefore, we use explicit waits (@browser.wait_until) after
# form submissions and navigation to ensure elements are present before assertions.
# This is the recommended pattern for modern web testing with async content.
...
describe 'Acceptance Tests' do
  ...
  describe 'Homepage' do
    ...
    describe 'Add Project' do
      ...
      it '(BAD) should not be able to add an invalid project URL' do
        ...
        # Wait for redirect and flash message to render (see note at top of file)
        @browser.wait_until { @browser.div(id: 'flash_bar_danger').exists? }
    ...
    describe 'Delete Project' do
      it '(HAPPY) should be able to delete a requested project' do
        ...
        # Wait for project to be created and redirected to project page (see note at top of file)
        @browser.h1(id: 'main_header').wait_until(&:present?)
        @browser.wait_until { @browser.url.include?(USERNAME) && @browser.url.include?(PROJECT_NAME) }

        # WHEN: they revisit the homepage and delete the project
        @browser.goto homepage

        # Wait for projects table and delete button to render (see note at top of file)
        @browser.table(id: 'projects_table').wait_until(&:present?)
        @browser.button(id: 'project[0].delete').wait_until(&:present?)
  ...
  describe 'Project Page' do
    it '(HAPPY) should see project content if project exists' do
      ...
      @browser.goto "#{homepage}/project/#{USERNAME}/#{PROJECT_NAME}"
    ...
    it '(HAPPY) should be able to traverse to subfolders' do
      ...
      # Wait for page content to render after navigation (see note at top of file)
      @browser.h1(id: 'main_header').wait_until(&:present?)
      ...
      models_folder = folder_rows.find do |row|
        row.tds.first.link.href.include? '/models'
      end

      models_folder.link.click

      # Wait for new page content to render after navigation (see note at top of file)
      @browser.h1(id: 'main_header').wait_until(&:present?)
      ...
      _(file_rows.count).must_equal 4
    end
    ...
    it '(BAD) should report error if subfolder does not exist' do
      ...
      @browser.goto "#{homepage}/project/#{USERNAME}/#{PROJECT_NAME}/bad_folder"
